...
DSN: `_busy_timeout=5000&_journal_mode=WAL&_secure_delete=on`

| Параметр | Зачем |
|----------|-------|
| `busy_timeout` | Второй экземпляр приложения ждёт, а не падает с `database is locked` |
| `journal_mode=WAL` | Устойчивость к падениям, параллельные чтения |
...
Права 0600 выставляются на файл БД и на WAL-файлы (`-wal`, `-shm`) до записи
данных.
